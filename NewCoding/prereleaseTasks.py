...
class Dastan:
    ...
    def __CreateCustomPlayers(self):
        player1Name = input("Player 1 enter name:").strip()
        player2Name = input("Enter player 2 name").strip()
        while player1Name == player2Name:
            print("cannot have both players with the same name")
            player2Name = input("EnterPlayer 2 name:")
        self._Players.append(Player(player1Name, 1))
        self._Players.append(Player(player2Name, -1))
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            Choice = 0
            UpperLimit = 3
            if self.__AwardWafr() and self._CurrentPlayer.getWafrAwarded() == False:
                UpperLimit = 7
                self._CurrentPlayer.setWafrAwarded(True)
            while Choice < 1 or Choice > UpperLimit:
                promptStr = "Choose move option to use from queue (1 to 3) or 9 to take the offer: "
                if UpperLimit == 7:
                    promptStr = "You have been awarded a Wafr, you can select any move from your queue for free this turn."
                Choice = int(input(promptStr))
                if Choice == 9:
...
class Player:
    ...
    def setWafrAwarded(self, new):
        self.__WafrAwarded = new
